...
retail_df$UnitPrice <- if_else(is.na(retail_df$UnitPrice),
...
retail_df$Quantity <- if_else(is.na(retail_df$Quantity),
                              round(mean(retail_df$Quantity, na.rm = TRUE)),
                              retail_df$Quantity)

# Final Missing Values Check 
final_missing_values <- sapply(retail_df, function(x) sum(is.na(x)))
...
duplicates <- retail_df %>%
  group_by(CustomerID, OrderDate, ProductDescription) %>%
  filter(n() > 1)
print(duplicates)
...
#3. Replace "St" with "Street" in the Address column
retail_df <- retail_df %>%
